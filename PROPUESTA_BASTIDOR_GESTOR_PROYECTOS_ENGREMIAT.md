...
**Construida** la sección "Regenerar grafo_visor.json" en `grafos.html`: muestra en real cuándo se generó por última vez el `grafo_visor.json` ya publicado (fecha + contadores reales, leídos en vivo del propio fichero), y un botón "Copiar los 3 comandos" que copia el ciclo completo real (`cd tools/gobierno/graphify_visor` → `node mapear_grafo_visor.mjs` → `node desplegar_visor.mjs`) -- regenerar en local y publicar en el mismo paso, usando el `copiarConFallback()` ya construido en §8.105 para este mismo fichero. Desplegado y verificado con `curl`: la sección y los 3 comandos reales aparecen tal cual en el HTML servido. **Sustituida por completo en §8.108** -- no era lo que el operador pedía de verdad.

### 8.108 Tercer paso real en Constructor de grafos: extracción en vivo, sin CLI, del mismo método que `mapear_grafo_visor.mjs`

Corrección del operador sobre la sección anterior: *"el objetivo de este espacio, es que yo pueda poner un html de nuestra web local y que el script extraiga el json, reformula"* -- el CLI+panel de §8.107 regeneraba el mapa COMBINADO de las 25 páginas (un fichero fijo, sin entrada), cuando lo pedido era poder pegar UNA página cualquiera y obtener SU grafo real, en el sitio.

**Redescubierto al repensar**: a diferencia de Ollama (§8.102) o de `proponer_candidato.mjs` (§8.103), esta extracción NO necesita ningún proceso local -- las dos expresiones regulares reales que usa `mapear_grafo_visor.mjs` (enlace real hacia otra página, llamada real a la API) son JavaScript puro sobre texto ya fetcheado; el navegador puede ejecutarlas él mismo, sin CLI, sin panel, en vivo. Eliminada la sección de §8.107 por completo.

**Construido**: la MISMA caja de "Constructor de grafos" gana un tercer paso real, en cascada -- (1) ¿es JSON directo?, (2) si no, ¿qué `.json` carga de verdad esa página? (§8.105), (3) si tampoco, `extraerGrafoDePagina()` construye un grafo real ahí mismo a partir de los enlaces y llamadas reales que la página declara en su propio código -- mismo método mecánico que `mapear_grafo_visor.mjs`, aplicado a una sola página en vez de a las 25 de golpe. Nunca inventa: una página sin enlaces ni llamadas reales produce un grafo de un solo nodo, sin aristas -- resultado real y honesto, no un error. El mapa combinado (`grafo_visor.json`) lo sigue manteniendo la tarea programada real (§8.94), mencionado explícitamente en el texto para no confundir los dos.
...
### 8.109 "Le falta detalle" era profundidad, no estilo: extracción recursiva real + cruce con el servidor

Corrección del operador sobre el §8.108, aclarando el feedback anterior: *"cuando digo que falta detalle me refiero a profundidad de nodos y otros elementos que ahora están descartados, necesitamos todos los elementos relacionados con un html ingresado"*. El malentendido real: §8.108 solo extraía el vecindario de UN salto (la página pegada y sus enlaces/llamadas directas) y omitía por completo la capa de `servidor_memoria.mjs` que sí tiene la versión completa (`mapear_grafo_visor.mjs`).

**Redescubierto antes de construir**: había una tercera capa real en `mapear_grafo_visor.mjs` que §8.108 no reproducía -- el cruce con los recursos reales del Sheet/Baserow (arista `toca`). Comprobado su contenido real (`estructura_sheet.json`/`estructura_baserow.json`): son el esquema real de datos del cliente (ID del Sheet, nombres reales de pestañas/tablas de un CRM de oportunidades comerciales) -- no secretos, pero sí datos de negocio del cliente. Decisión tomada sin preguntar (mismo criterio de prudencia que ya rige credenciales/tokens en este proyecto): esa capa se deja fuera de esta caja de uso general, con el motivo explícito en el propio texto -- el mapa completo con esa capa sigue disponible en `grafo_visor.json` (§8.94).

**Construido**: `extraerGrafoRelacionado()` sustituye a la versión de un solo salto -- un recorrido real (BFS, tope de seguridad real de 40 páginas para nunca rastrear sin fin) que sigue cada página nueva que va descubriendo, no solo la primera, acumulando sus enlaces y llamadas reales a la API. Además, cuando hay endpoints reales encontrados, se trae `servidor_memoria.mjs` (comprobado antes que su código fuente no contiene ningún secreto -- las credenciales se leen de variables de entorno/ficheros aparte, nunca están en el fichero) y cruza cuáles de esos endpoints sirve de verdad, añadiendo el nodo `servidor` y sus aristas `sirve` reales -- mismo criterio mecánico que `mapear_grafo_visor.mjs`, acotado solo a lo relevante para esta página, nunca el listado completo del servidor sin venir a cuento. Añadido el volumen real `servidor_memoria.mjs:ro` a `docker-compose.yml` de `graphify-visor` (antes solo se montaba con permiso de escritura en `memoria-montaje`; aquí de solo lectura, mismo fichero real).
...
### 8.110 Cerrar el círculo real: crear → configurar → promover → guardar, todo en vivo desde Constructor de grafos

Pedido en dos mensajes: *"simplifica y añade un botón para promover al candidato, necesitamos poder configurar el espacio y el nombre"*, y después, explícito: *"necesitamos que el candidato que podamos configurar lo podamos validar desde constructor de grafos, necesitamos cerrar el círculo, desde la creación por un html, la configuración del grafo y poder almacenarlo una vez aprobado"*.

**Investigado antes de construir**: el endpoint real `/api/promover_grafo` (§8.95, ya usado por el botón "Promover" de "A promover") existía pero tenía `espacioReal` fijo a `null` en el propio servidor -- nunca configurable, ni por CLI ni en vivo. Corregido `promoverGrafoReal()`/el handler HTTP en `servidor_memoria.mjs` para aceptar `espacioReal` real del cuerpo de la petición. **Bug de despliegue real encontrado al verificar**: `desplegar_visor.mjs` apunta por defecto al servicio `graphify-visor` -- el primer despliegue de este cambio de backend NO tocó `memoria-montaje` (el servicio que de verdad corre `servidor_memoria.mjs`), así que la llamada en vivo habría seguido usando el código viejo sin decir nada. Corregido ejecutando explícitamente `node desplegar_visor.mjs --servicio memoria-montaje`.

**Construido**: la caja de "proponer" se simplifica y se convierte en la de "promover" -- desaparecen los campos `id` (ahora se deriva solo del nombre) y `prioridad` (el endpoint real de promover nunca lo usó, solo tenía sentido para el borrador de `proponer_candidato.mjs`); aparece un campo nuevo real `espacioReal` (opcional, ej. "n8n"). Un único botón, "Promover como grafo real", llama en vivo a `/api/promover_grafo` -- sin CLI, sin comando que copiar. Como un grafo extraído en vivo no tiene página visor propia, su `pagina` real guardada es `grafos.html?url=<la URL original>` -- al abrir esa ficha desde la galería, `grafos.html` lee el parámetro real de la URL, rellena la caja y vuelve a generar la misma extracción en vivo, siempre actualizada, sin tener que persistir el JSON en ningún sitio.
...
### 8.112 Cuatro espacios reales en Constructor de grafos: grafo, leyenda, ficha del grafo, formulario

Pedido: *"al generar vista previa construir la ficha del grafo a través de la plantilla, de esta forma tendríamos... 1. el grafo. 2. la leyenda real del grafo. 3. la ficha del grafo. 4. formulario para configurar y promover grafo, necesitamos que mantenga el estilo que seguimos trabajando"*.

**Construido**: `construirFichaGrafo()` calcula, EN VIVO y sobre los nodos/aristas ya generados (nunca texto libre inventado), las respuestas reales a las mismas 5 preguntas que `plantilla_extractor.mjs` documenta desde el §8.111 -- (1) recuento real de nodos por tipo, (2) recuento real de aristas por relación, (3) de qué fuente real sale (JSON directo / descubierto / extraído en vivo, con la URL real), (4) huecos/externos reales sin resolver, (5) límite honesto real (tope de páginas si se extrajo en vivo, o remisión a la ficha real del JSON si es directo). Se renderiza como cuarto bloque real del panel, entre la leyenda y el formulario -- mismo estilo ya establecido (`h3`/`p` de `plantilla_grafo_espacio.html`, ver §8.108) -- y la misma ficha (concatenada) pasa a ser la `descripcion` real que se guarda al promover, sustituyendo al texto genérico anterior. La ficha por nodo al clic (bonus ya existente) se reubica al final del panel, como detalle secundario tras el formulario.
...
### 8.116 `arquitecto.html` -- constructor dinámico de páginas, operacionalizando "Índice"

Pedido, tras un ciclo de aclaración (una pregunta directa sobre la función real de "Arquitecto" fue descartada primero, contestada después): *"el objetivo de arquitecto es crear un constructor dinámico de páginas html y sus cajas... usar el sistema panel de campaña para construir elementos html, valora y mejora la propuesta"*.

**Mejora aplicada antes de construir**: ninguna página web puede escribir un fichero `.html` nuevo en el disco del operador -- mismo límite real ya confirmado con el worker local (§8.102) y `mapear_grafo_visor.mjs` (§8.107), no resoluble con permisos de red. Se separó la propuesta en dos pasos reales, nunca uno fingiendo al otro: **(1)** creación en vivo, real, de la estructura (Producto/Procesos/Tareas) dentro de `PRO-0002 Índice` vía `/api/crear_registro` -- esta parte SÍ puede ser 100% en vivo, ya con el bug de §8.115 corregido; **(2)** generación real del HTML de partida (una `<section>` por caja, con las funciones reales como `<li>TODO:>` de partida, misma plantilla visual ya establecida) para copiar -- con los pasos manuales reales que faltan (guardar el fichero, montar el volumen en `docker-compose.yml`, enlazar desde `home.html`, rellenar los `TODO`, desplegar) listados explícitos en la propia página, nunca omitidos.

**Construido** `arquitecto.html`: formulario real (nombre de la página + fichero real, con slug automático) y una lista dinámica de cajas (cada una con su nombre real y sus funciones reales, una por línea) -- arranca con una caja real vacía, nunca cero. Al construir: crea de verdad el Producto bajo `PRO-0002`, un Proceso real por caja, una Tarea real por función (sin `ESTADO` forzado -- quedan con el valor real por defecto, "Pendiente", porque la página todavía no existe de verdad); genera el HTML real y muestra los 5 pasos reales que faltan. Enlazada desde `home.html` (7ª pieza real) y añadido su volumen real a `docker-compose.yml`.

**Verificado sin contaminar datos reales de producción**: sintaxis del script comprobada con `node`/`vm.Script`; la función de generación de HTML (`generarHtmlReal`/`slug`) verificada de forma aislada con `node` (sin llamadas reales) -- confirma secciones y `TODO` correctos. Desplegado y confirmado con `curl` que los campos reales del formulario están presentes. Deliberadamente **no** se ejecutó una creación real de prueba contra el Sheet CORE -- no existe forma de borrar una fila real después, y dejar un Producto de prueba en `PRO-0002` sería justo el tipo de dato falso que este proyecto evita en todas partes. El primer uso real de "construir página real" queda para cuando el operador tenga una página real que dar de alta.

**Mejora inmediata**: pedido *"añade un menú desplegable (con las páginas de home) para elegir de dónde cuelga la nueva página creada, por defecto la opción seleccionada será home, esto se tiene que crear de forma dinámica"*. Construido el desplegable "¿De dónde cuelga?", poblado en vivo leyendo `/api/jerarquia_campanas` y extrayendo las páginas reales que ya son Producto dentro de `PRO-0002 Índice` (nunca una lista fija a mano -- crece sola según se dan de alta páginas nuevas con el propio Arquitecto). `home.html` no es un Producto real de Índice (es la raíz) -- se añade siempre a mano como primera opción real y por defecto, tal como se pidió. La selección se refleja en tres sitios reales: la `DESCRIPCION` real del Producto creado ("Página real colgada de X"), el enlace real de "volver" del HTML generado (apunta a la página elegida, no siempre a `home.html`), y el paso real de "enlazar" en las instrucciones (nombra la página exacta y da el fragmento `<a href="...">` real a copiar). Verificado con `node` contra el VPS real: el desplegable resuelve correctamente las 6 páginas reales de Índice más `home.html`.

**Corrección de pulido**: pedido *"quita esto, no lo necesitamos"* sobre el campo visible "Fichero real (.html)", aclarado después como *"de la vista"* -- se quita solo de la interfaz visible, el nombre de fichero real se sigue derivando internamente con `slug(nombrePagina) + '.html'` (mismo criterio ya usado antes de que existiera el campo editable). Al quitar el `<input>` quedó colgando la función `sugerirArchivo()` y su `addEventListener` a nivel superior, que habría lanzado un `TypeError` en la carga de la página (elemento inexistente) rompiendo todo el script -- localizado con `grep` antes de desplegar y limpiado junto con la lectura de `construirPagina()`. Verificado con `node -e "new vm.Script(...)"` (mismo patrón de chequeo de sintaxis ya usado en este proyecto) antes de desplegar, y con el navegador real tras el despliegue: la página carga sin errores de consola y todos los controles (desplegables, botón "+ Añadir caja") responden.

**Ampliación real**: pedido *"añade otro menú desplegable dinámico inferior a nombre de la página con los tipos que tenemos estandarizados: esta pieza nos servirá para definir el tipo de html según nuestro estándar"*. Añadido el desplegable "Tipo real" reutilizando literalmente `TIPOS_REALES` de `grafos.html` (línea 114: Espacio/Personaje/Recurso/Módulo/Herramienta/Transversal) -- nunca una lista distinta inventada aquí. La selección se refleja en la `DESCRIPCION` real del Producto creado y en el mensaje de estado final, igual que ya hace `colgarDe`. Verificado en el navegador real: el desplegable renderiza las 6 opciones reales con "Espacio" por defecto.

**Mejora real (asesor técnico)**: pedido *"añadir otro menú desplegable para elegir qué tipo de caja queremos construir, investiga sobre elementos html básicos para configurar mejor las cajas"*. Antes de construir, revisados los patrones de HTML básico ya usados de verdad en otras páginas del propio visor para no inventar nada nuevo: tabla real con `<thead>`/`<tbody>` inline (`panel_operativo.html`, línea 157), grid de tarjetas `.menu`/`a.item` (`home.html`), y el propio patrón `label`+`input` por fila que ya usa este mismo Arquitecto. Con eso, cada caja ahora elige un **tipo real** entre cinco -- Lista (`<li>`, comportamiento previo), Texto (`<p>` sueltos), Tabla (primera línea = cabeceras reales separadas por `|`, resto = filas reales), Tarjetas (`Título - descripción` por línea → `.menu`/`a.item`), Formulario (`label`+`input` de partida por línea) -- con una ayuda visible que cambia en vivo según el tipo elegido (`actualizarAyudaCaja()`). `generarHtmlReal()` reparte el contenido real por tipo (`generarContenidoCaja()`) y solo añade el CSS extra de Tarjetas/Formulario al HTML generado cuando de verdad hay una caja real de ese tipo -- nunca CSS muerto. Verificado en aislado con `node`+`vm` (las 5 ramas generan HTML real correcto sobre datos de ejemplo) y en el navegador real: el desplegable de tipo cambia la ayuda en vivo (probado con Tabla).

### 8.118 Cerrar el segundo círculo: automatizar de verdad 3 de los 5 "pasos reales que faltan"

Pedido, viendo el resultado real de crear `PRD-0008` ("mapa"): *"construye el mecanismo para crear estos nuevos procesos de forma dinámica"* -- automatizar los pasos manuales que quedaban tras "Construir página real". Analizado antes de tocar nada: de los 5 pasos, 3 son mecánicos y seguros de automatizar (guardar el fichero, añadir la línea de volumen, desplegar+verificar); 2 necesitan criterio real del operador y NO se automatizan a ciegas (enlazar desde la página elegida -- cada página objetivo tiene su propia estructura real, insertar a ciegas podría romperla --, y rellenar los `TODO` con contenido de verdad, que nadie más que el operador puede inventar de forma honesta).

**Diseño real, respetando la Puerta Humana**: el navegador sigue sin poder escribir ficheros ni tocar `docker-compose.yml` -- eso no cambia. En vez de dársela a `servidor_memoria.mjs` (un servicio de red expuesto, ya real, ya escribiendo Sheets/JSON -- pero jamás se le ha dado la capacidad de escribir código/infraestructura, y no se le da ahora), `construirPagina()` ahora guarda el HTML real generado en una cola real (`POST /api/pagina_pendiente`, fichero `paginas_pendientes.json` en `DATOS_DIR`, mismo tipo de escritura ya autorizada que `memoria_compartida.json` -- datos, nunca código ni infraestructura). Un script CLI nuevo, `aplicar_pagina_arquitecto.mjs` -- que corre el operador (o Claude, a petición explícita) en terminal, nunca la página web -- recoge la entrada por red (`GET /api/pagina_pendiente?archivo=X`), escribe el fichero real (rechaza sobrescribir si ya existe, "nunca a ciegas"), inserta la línea de volumen real en `docker-compose.yml` (mismo escaneo por indentación que ya usa `desplegar_visor.mjs`, verificado antes en una copia de prueba), y termina llamando al propio `desplegar_visor.mjs` -- reutilizado, no reinventado. Al terminar, borra la entrada de la cola (`DELETE /api/pagina_pendiente`) e imprime los 2 pasos reales que sí quedan para el operador.

Si la cola falla al guardar (servidor caído), `arquitecto.html` no pierde nada: el HTML completo sigue en el cuadro para copiar a mano, y los 5 pasos originales reaparecen como plan B explícito.

**Verificado de extremo a extremo, con limpieza completa**: probada la inserción de volumen en una copia temporal de `docker-compose.yml` antes de tocar la real; desplegado `memoria-montaje` con `--servicio` explícito; hecha una prueba real completa con una página desechable (`prueba_arquitecto_test.html`, `productoId` ficticio `TEST-0000`, sin tocar el Sheet real) -- cola → CLI → fichero real → volumen real → despliegue real → `curl` 200 real, todo en un solo comando. Confirmado que la entrada de la cola se borró sola al terminar. Deshecho el rastro completo después: fichero local borrado, línea de `docker-compose.yml` revertida, redeploy para recrear el contenedor sin el volumen, y borrado a mano el fichero huérfano que quedaba en el host del VPS (el `scp` solo añade, nunca borra) -- verificado `curl` 404 real al final.

**Aplicado de verdad sobre `PRD-0008` (pedido "hazlo tú")**: `PRD-0008` ("mapa.html") se había creado ANTES de que existiera la cola, así que su HTML nunca quedó guardado en ningún sitio real -- solo se vio una vez en pantalla. En vez de inventar su contenido, se leyó la estructura real ya existente en el Sheet (`/api/ficha` para `NOMBRE`/`DESCRIPCION` -- confirmó `colgarDe=home.html`, `tipo=Espacio` -- y `/api/jerarquia_campanas` para sus Procesos/Tareas reales: un único Proceso real "PRESENTACIÓN" con una Tarea real "texto para explicar la identidad de la pagina"). Con esos datos reales (nunca inventados) se regeneró el HTML llamando a la misma `generarHtmlReal()` de `arquitecto.html` (extraída y ejecutada en aislado con `node`+`vm`, mismo patrón de verificación ya usado en este mismo punto), se encoló vía `POST /api/pagina_pendiente` y se aplicó con `node aplicar_pagina_arquitecto.mjs mapa.html` -- desplegado y verificado con éxito (`curl` 200). De los dos pasos que quedan siempre a mano, se hizo también el enlace real desde `home.html` (mismo patrón de tarjeta `a.item`, sin inventar nada -- 8ª pieza real) porque es mecánico y no necesita contenido inventado; el `TODO` real de la página se deja explícitamente sin rellenar, porque nadie ha dicho todavía qué debe mostrar "Mapa" de verdad -- inventarlo habría sido la excepción exacta que este proyecto evita.

### 8.119 Fase 1 del segundo círculo: fragmento real + reconstrucción, probado sobre `mapa.html`

Pedido, tras valorar (como asesor técnico) un drag-and-drop de tarjetas Producto/Proceso para configurar la jerarquía real de las páginas: hacerlo en dos fases, empezando por *"el mecanismo de fragmento + reconstrucción, probado sobre mapa.html (su única caja real, 'PRESENTACIÓN') -- cierra su propio TODO de una vez, con un editor real en vez de contenido inventado"*.

**El hueco real identificado antes de construir**: el Sheet y el `.html` desplegado solo estaban conectados UNA VEZ, en el momento de crear la página (`aplicar_pagina_arquitecto.mjs`). Después de eso, el Sheet no sabía nada del contenido real de una caja -- solo su `NOMBRE`. Sin arreglar esto, cualquier "reconstrucción" futura (fase 2 incluida) volvería siempre al `TODO` de partida, perdiendo cualquier contenido real ya escrito.

**Construido**: contenido real de cada caja (Proceso) guardado aparte del Sheet, en una cola nueva y propia (`fragmentos_cajas.json` en `DIR_DATOS`, mismo tipo de escritura de datos ya autorizada -- `GET`/`POST /api/fragmento_caja` en `servidor_memoria.mjs`). Nueva sección en `arquitecto.html`, "Editar contenido real de una página existente": desplegable con las páginas reales de Índice (reutiliza la misma búsqueda de `obtenerIndiceReal()`, extraída como función compartida para no duplicarla con el desplegable "¿de dónde cuelga?"), y por cada Proceso real de la página elegida, el mismo editor tipo/textarea/ayuda ya construido en la fase anterior (`TIPOS_CAJA`), precargado con su fragmento real si existe. "Guardar y reconstruir" guarda cada fragmento real, llama a la misma `generarHtmlReal()` ya existente (nunca duplicada) y encola el resultado con un flag nuevo `reconstruccion: true` -- distingue una reconstrucción intencional de una página que YA existe de verdad, de una colisión accidental con una página nueva. `aplicar_pagina_arquitecto.mjs` ahora solo permite sobrescribir un fichero real existente cuando ese flag viaja explícito; sin él, sigue rechazando como siempre ("nunca a ciegas").

**Verificado de extremo a extremo sobre el caso real** (`PRD-0008`/`mapa.html`, su único Proceso real "PRESENTACIÓN"): guardado un fragmento de prueba marcado explícitamente como tal, reconstruida la página con `node aplicar_pagina_arquitecto.mjs mapa.html` (confirmado en el log real "reconstruyendo página real existente"), verificado con `curl` que el contenido de prueba llegó de verdad a la página desplegada -- y devuelto todo a su estado honesto real después (fragmento vacío, página reconstruida de nuevo mostrando su `TODO` real de partida), para no dejar contenido inventado permanente en una página real. `mapa.html` queda ahora editable de verdad desde `arquitecto.html`, sin que el operador dependa de pedírmelo a mí.
