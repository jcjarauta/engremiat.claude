...
const EXPECTED_MODULE_COUNTS = { CORE: 78, GANTT: 3, ECONOMICO: 1, IMPACTO: 1, COMPRAS: 7, CONVOCATORIAS: 2, CLIENTE: 3, VENTAS: 3, OPORTUNIDAD: 2 };
...
export function validatePackageMap(map) {
  const errors = [];
  if (!map || map.schemaVersion !== 1 || map.universeExpected !== 174 || !Array.isArray(map.entries)) {
    errors.push('CABECERA_MATRIZ_INVALIDA');
    return errors;
  }
  if (map.entries.length !== 174) errors.push(`UNIVERSO_ESPERADO_174 actual=${map.entries.length}`);
  ...
  const expectedCounts = { production: 100, test: 9, auxiliary: 37, excluded: 28, mixed: 0 };
