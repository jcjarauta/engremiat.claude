...
export function validatePackageMap(map) {
  const errors = [];
  if (!map || map.schemaVersion !== 1 || map.universeExpected !== 134 || !Array.isArray(map.entries)) {
    errors.push('CABECERA_MATRIZ_INVALIDA');
    return errors;
  }
  if (map.entries.length !== 134) errors.push(`UNIVERSO_ESPERADO_134 actual=${map.entries.length}`);
  ...
  const expectedCounts = { production: 63, test: 7, auxiliary: 35, excluded: 24, mixed: 5 };
