...
const EXPECTED_MODULE_COUNTS = { CORE: 60, GANTT: 3, ECONOMICO: 1, IMPACTO: 1, COMPRAS: 7, CONVOCATORIAS: 2, CLIENTE: 3, VENTAS: 3, OPORTUNIDAD: 2 };
...
export function validatePackageMap(map) {
  const errors = [];
  if (!map || map.schemaVersion !== 1 || map.universeExpected !== 157 || !Array.isArray(map.entries)) {
    errors.push('CABECERA_MATRIZ_INVALIDA');
    return errors;
  }
  if (map.entries.length !== 157) errors.push(`UNIVERSO_ESPERADO_157 actual=${map.entries.length}`);
  ...
  const expectedCounts = { production: 82, test: 9, auxiliary: 38, excluded: 28, mixed: 0 };
  ...
  if (htmlA.length !== 23) errors.push(`HTML_A_ESPERADOS_23 actual=${htmlA.length}`);
