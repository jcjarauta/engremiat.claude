...
const EXPECTED_MODULE_COUNTS = { CORE: 72, GANTT: 3, ECONOMICO: 1, IMPACTO: 1, COMPRAS: 7, CONVOCATORIAS: 2, CLIENTE: 3, VENTAS: 3, OPORTUNIDAD: 2 };
...
export function validatePackageMap(map) {
  const errors = [];
  if (!map || map.schemaVersion !== 1 || map.universeExpected !== 168 || !Array.isArray(map.entries)) {
    errors.push('CABECERA_MATRIZ_INVALIDA');
    return errors;
  }
  if (map.entries.length !== 168) errors.push(`UNIVERSO_ESPERADO_168 actual=${map.entries.length}`);
  ...
  const expectedCounts = { production: 94, test: 9, auxiliary: 37, excluded: 28, mixed: 0 };
