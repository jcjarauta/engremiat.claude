...
const EXPECTED_MODULE_COUNTS = { CORE: 62, GANTT: 3, ECONOMICO: 1, IMPACTO: 1, COMPRAS: 7, CONVOCATORIAS: 2, CLIENTE: 3, VENTAS: 3, OPORTUNIDAD: 2 };
...
export function validatePackageMap(map) {
  const errors = [];
  if (!map || map.schemaVersion !== 1 || map.universeExpected !== 159 || !Array.isArray(map.entries)) {
    errors.push('CABECERA_MATRIZ_INVALIDA');
    return errors;
  }
  if (map.entries.length !== 159) errors.push(`UNIVERSO_ESPERADO_159 actual=${map.entries.length}`);
  ...
  const expectedCounts = { production: 84, test: 9, auxiliary: 38, excluded: 28, mixed: 0 };
  ...
  if (htmlA.length !== 24) errors.push(`HTML_A_ESPERADOS_24 actual=${htmlA.length}`);
