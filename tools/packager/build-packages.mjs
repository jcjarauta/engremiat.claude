...
const EXPECTED_MODULE_COUNTS = { CORE: 58, GANTT: 3, ECONOMICO: 1, IMPACTO: 1, COMPRAS: 6, CONVOCATORIAS: 2 };
...
export function validatePackageMap(map) {
  const errors = [];
  if (!map || map.schemaVersion !== 1 || map.universeExpected !== 143 || !Array.isArray(map.entries)) {
    errors.push('CABECERA_MATRIZ_INVALIDA');
    return errors;
  }
  if (map.entries.length !== 143) errors.push(`UNIVERSO_ESPERADO_143 actual=${map.entries.length}`);
  ...
  const expectedCounts = { production: 71, test: 8, auxiliary: 37, excluded: 27, mixed: 0 };
  ...
  if (htmlA.length !== 21) errors.push(`HTML_A_ESPERADOS_21 actual=${htmlA.length}`);
