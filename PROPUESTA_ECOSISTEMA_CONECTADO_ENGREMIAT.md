...
|---|---|---|---|
| **C0** | ~~Portainer (servidor VPS + agente) y Headplane~~ | Bajo — ninguna ruta de datos nueva | **HECHO y verificado por completo 2026-09-02**: Portainer real en `https://100.107.171.88:9443` con dos entornos reales — `primary` (VPS, 9/9 contenedores) y `pi-nucleo` (`tcp://100.125.52.52:9001`, conectado en cuanto la Pi volvió a estar encendida). Headplane real en `http://100.107.171.88:3002/admin`, sesión iniciada. Dos hallazgos reales en el despliegue: (1) Headplane exige Headscale ≥0.27.0, la Etapa 1 corría 0.23.0 — subido a 0.29.3, migración de esquema automática, "Connected to Headscale 0.29.3" confirmado en logs; (2) la clave de API generada antes de esa subida quedó invalidada por el propio cambio de version (formato de clave distinto) — regenerada después de subir Headscale y verificada con curl antes de dársela al operador |
