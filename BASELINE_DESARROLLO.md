...
| H | UI, panel e informes | **En curso — bug crítico corregido** | Se encontró y corrigió un bug real (no solo lentitud): `obtenerPanelOperativo()`/`generarInforme()` devolvían objetos `Date` crudos que rompían la serialización de `google.script.run` silenciosamente, dejando el Panel operativo y los Informes colgados sin error visible. Corregido con `src/SerializacionService.js` + reescritura de `aplanarInformeParaExportar_` (exportación CSV/PDF). Verificado visualmente por el usuario: Panel, Informes, CSV y PDF funcionan. Pendiente: formularios (modal 420×520), resto de tipos de informe, estabilidad general |
