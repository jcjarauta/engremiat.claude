...
- Paso 7 del roadmap (Fase H, UI) cerrado: 3 bugs críticos encontrados y corregidos (serialización de fechas en Panel/Informes, mapa `ENTIDAD_DOCUMENTO_A_MVP` desincronizado, `visibleSi` con valores de catálogo inventados en Tarea/Proceso). Checklist completa verificada por el usuario en Apps Script real: Panel, 3 tipos de Informe + CSV/PDF, 5 formularios principales.
...
| F | Documentos, decisiones, incidencias | **Cerrada (bloque DOCUMENTO)** | 6 reglas implementadas y verificadas (`FUNC-DOC-001` a `FUNC-DOC-006`): FK polimórfica huérfana/inactiva, doble vigencia, duplicado exacto, formato de VERSION y URL. El mapa `ENTIDAD_DOCUMENTO_A_MVP` desincronizado (detectado aquí, corregido en Fase H tras un bug de usuario real) hacía que `FUNC-DOC-001`/`002` nunca detectaran nada en documentos reales — corregido y re-verificado, ver Fase H |
...
| H | UI, panel e informes | **Cerrada — 3 bugs críticos encontrados y corregidos** | (1) `obtenerPanelOperativo()`/`generarInforme()` devolvían `Date` crudos, rompiendo `google.script.run` silenciosamente — corregido con `src/SerializacionService.js`. (2) `ENTIDAD_DOCUMENTO_A_MVP` usaba claves en mayúsculas sin tilde que nunca coincidían con el texto real del catálogo, dejando el selector "Registro" del formulario Documento siempre vacío (y `FUNC-DOC-001`/`002` sin detectar nada real) — corregido, mapa unificado entre Formularios.js e IntegrityService.js. (3) `visibleSi` de TAREA/PROCESO usaba valores de catálogo inventados (`'En curso'`, `'Terminado'`) en vez de los reales (`'En proceso'`, `'Completado'`), ocultando el campo `FECHA_INICIO_REAL` requerido y bloqueando el guardado. Los tres verificados end-to-end por el usuario en Apps Script real: Panel, 3 tipos de Informe + CSV/PDF, formularios de Tarea, Documento, Decisión, Incidencia y Material (estos tres últimos sin bugs adicionales). Riesgo residual: ~10 formularios restantes (Campaña, Proyecto, Producto, Proceso, Proveedor, Persona/Equipo, relaciones N:M) no verificados manualmente — mismo patrón de bug podría repetirse ahí |
...
| J | Seguridad, despliegue, baseline | **Cerrada** | Scope `drive` reducido a `drive.file` (único uso real: crear archivos nuevos, nunca leer/modificar existentes). Hashes SHA-256 (`HASHES_CIERRE.md`), acta de cierre (`ACTA_CIERRE_SESION.md`) y backup formal del Sheet (copia manual del usuario, `1IfjoIJPBQvnPoFDwmdHPsYbhQkuyZ15HO8ra62UGRwg`, verificado con 19 hojas) — todos completados. Reconciliación manual de la matriz de cobertura completada: 62/62 reglas `FUNC-*` verificadas, único gap real (`FUNC-TAREA-008`) cerrado con `auditarFaseC04B_TerminadaSinDuracionReal`, `result=OK` en Apps Script real |
