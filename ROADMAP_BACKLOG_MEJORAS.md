...
### O-deuda — Migración de columnas nuevas a hojas existentes ✅ NO ERA DEUDA (corregido 2026-08-18)

Se creyó, durante buena parte de la sesión, que `instalarEstructuraInicial()` no migraba columnas nuevas a hojas ya existentes -- de ahí que varios gaps (`CLIENTE_ID` en `02_PROYECTOS`, `TIPO_DOCUMENTO`/`Imagen`+`Vídeo`, hojas `45_TAREA_COMPETENCIA`/`46_TAREA_RECURSO_NECESIDAD`/`92_ESCENARIOS`) se resolvieran a mano vía Sheets API en vez de re-ejecutar el instalador. **Falso**: `instalarEstructuraInicial()` ya migra columnas faltantes al final de cada hoja (sin reordenar, resuelve por nombre de cabecera) y repara los rangos con nombre de catálogo (`asegurarRangosNombradosCatalogo_`) -- solo hacía falta volver a ejecutar el menú "Instalar estructura inicial (hojas + catálogo)". Confirmado al re-ejecutarlo tras añadir `PERSONA_EQUIPO.TELEGRAM_CHAT_ID`/`NIVEL_PERMISO_BOT`: migró `ORIGEN_CREACION` a las 43 hojas de entidad que lo tenían pendiente (gap sistémico que no se había detectado hasta ahora), `CLIENTE_ID` también en `13_INCIDENCIAS` (no solo en `02_PROYECTOS`), `CAPACIDAD`/`CAPACIDAD_UNIDAD` en `23_RECURSO`, creó la hoja `93_MAPEO_IDS_TEMPORALES` que faltaba, y aseguró 71 rangos con nombre. Sin trabajo nuevo que hacer -- el hábito correcto de aquí en adelante es re-ejecutar ese menú tras cualquier cambio de esquema, en vez de editar el sheet a mano.
